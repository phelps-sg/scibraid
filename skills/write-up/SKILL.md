...
2. **Start the draft.** `scibraid draft start <dir> <slug> [<slug> ...] --focus "<the steer>" [--voice <name>]` writes three files and a record. `dossier.md` is the evidence: every hypothesis with the observations for and against it, the conditions each was obtained under, the passage it rests on and its citation key; the results recorded as contradicting each other; how hypotheses in different subgraphs bear on one another; the leads with their checks; and how the review was made. `refs.bib` holds a reference for every paper, generated from cached metadata. `main.tex` is a skeleton arXiv accepts. Running the command again refreshes the dossier and the references and leaves `main.tex` alone.
...
**Only what the dossier holds.** A claim about a paper's result needs that result in the dossier, cited with its key. Take numbers from the quoted passages, not from memory of the paper; if the passage does not give the number, read the attached text (`scibraid paper show <id>` says whether there is one; `scibraid fetch <id>` gets one) or leave the number out.
