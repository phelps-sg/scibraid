...
4. Check that each paper's evidence is connected to the hypotheses in contention and not only to a restatement of its own abstract. For any result the question turns on, read the source yourself and correct the extraction if it is wrong; an extractor's confident misreading is the error most worth catching here.
